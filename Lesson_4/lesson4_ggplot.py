...
def lineplot(hr_year_csv):
    ...
    gg = ggplot(data, aes(x = "yearID", y = "HR")) + geom_point(colour = "red") + geom_line(colour = "red") \
         + ggtitle("Home Runs by Year") + xlab("Year") + ylab("HR")

    return gg


csv_name = "hr_year.csv"

print lineplot(csv_name)
...
def lineplot_compare(hr_by_team_year_sf_la_csv):
    ...
    gg = ggplot(data, aes(x = "yearID", y = "HR", color = "teamID")) + geom_point() + geom_line() +\
         ggtitle("Total HR per Year")
    return gg


csv2 = "hr_by_team_year_sf_la.csv"
print lineplot_compare(csv2)
