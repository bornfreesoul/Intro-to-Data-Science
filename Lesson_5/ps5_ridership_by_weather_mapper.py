...
def mapper():
    ...
    for line in sys.stdin:
        ...
        key = format_key(float(tokens[14]), float(tokens[15]))
        #logging.info("{0}\t{1}\t{2}".format(tokens[14], tokens[15], key))
        #logging.info("{0}\t{1}".format(key, tokens[6]))
        print "{0}\t{1}".format(key, tokens[6])
